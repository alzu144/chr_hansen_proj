...
from Filtering import MICC, FIREMIC

DATA_PATH = PROJECT_DIR + "Data/Processed/"

parser = argparse.ArgumentParser(
    description="Filter-based methods to select the most relevant feature subset"
)
parser.add_argument(
...
parser.add_argument(
    "-m", "--method", type=str,
    help="the filter method to use; must be one of:\n"
        + " - MICC\n"
        + " - FIREMIC"
)
parser.add_argument(
    "-s", "--size", type=float,
    help="final size of the filtered feature set as a proportion of the original feature set"
)
...
# filtering
if cli_args.method == "MICC":
    filtering_algorithm = MICC
elif cli_args.method == "FIREMIC":
    filtering_algorithm = FIREMIC


print("Started filtering.")
subset_indexes = filtering_algorithm(X_train, y_train, cli_args.size)
print("Finished filtering.\n")

kmer_data_filtered = kmer_data.iloc[:, subset_indexes]
